...
def create_tables():
    ...
    cur.execute("select exists(select * from information_schema.tables where \
                 table_name=%s)", ('features',))
    if not cur.fetchone()[0]:
        cur.execute("CREATE TABLE features (site_id integer, \
                     cluster_id integer, countries varchar);")

    #create table clusters    
    cur.execute("select exists(select * from information_schema.tables where \
                 table_name=%s)", ('clusters',))
    if not cur.fetchone()[0]:
        cur.execute("CREATE TABLE clusters (site_id integer, \
                     cluster_id integer, countries varchar);")
        
    #create table relations    
    cur.execute("select exists(select * from information_schema.tables where \
                 table_name=%s)", ('relations',))
    if not cur.fetchone()[0]:
        cur.execute("CREATE TABLE relations (site_id_from integer, \
                     site_id_to integer);")
        
    #create table clusterwordvecs    
    cur.execute("select exists(select * from information_schema.tables where \
                 table_name=%s)", ('clusterwordvecs',))
    if not cur.fetchone()[0]:
